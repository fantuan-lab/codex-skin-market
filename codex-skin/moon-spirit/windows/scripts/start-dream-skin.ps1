...
if (Test-Path -LiteralPath $StatePath) {
  ...
  if (-not (Stop-RecordedMoonSpiritInjector $old $Injector)) {
...
if (-not $verified) {
  ...
  if (Stop-RecordedMoonSpiritInjector $failedState $Injector) {
    Remove-Item -LiteralPath $StatePath -Force
  }
  throw '月影灵编已启动，但注入验证失败；本次启动的皮肤监视器已安全停止。请查看 injector.log 与 injector-error.log。'
}
Write-Host "月影灵编已生效（官方 Store Codex；CDP 仅监听 127.0.0.1:$Port）。"
