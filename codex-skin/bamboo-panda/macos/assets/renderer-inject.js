((cssText, artDataUrl, themeConfig) => {
  ...
  const applyTheme = (root, shell) => {
    ...
    const accent = colors.accent || "#3f5b2a";
    const accentAlt = colors.accentAlt || accent;
    const secondary = colors.secondary || "#a8b77b";
    const highlight = colors.highlight || "#c58c45";
    ...
    if (shell === "light") {
      ...
      variables = {
        ...
        "--ds-text": "#26321e",
        "--ds-muted": "#6e725f",
        "--ds-line": colors.line || "rgba(93, 116, 56, .24)",
      };
    } else {
      variables = {
    ...
    root.style.setProperty("--dream-skin-name", cssString(THEME.name || "竹影熊猫"));
    root.style.setProperty("--dream-skin-tagline", cssString(THEME.tagline || "跟着熊猫一起，把灵感、代码与效率慢慢做好。"));
  ...
  const ensure = () => {
    ...
    chrome.querySelector(".dream-skin-brand b").textContent = THEME.name || "竹影熊猫";
    chrome.querySelector(".dream-skin-brand small").textContent = THEME.brandSubtitle || "BAMBOO PANDA CODEX SKIN";
    chrome.querySelector(".dream-skin-status span").textContent = THEME.statusText || "PANDA FOCUS +1";
    chrome.querySelector(".dream-skin-quote").textContent = THEME.quote || "今天先把项目啃下来";
