...
1. Home route shows the Bamboo Panda image banner, two to four native suggestion cards, the real project selector/button, and native composer. The image already contains the branded headline, so duplicate generated hero copy stays hidden.
...
7. Restore removes live DOM/CSS, restores the saved base-theme values, closes the CDP session after restart, and supports later reinstallation.
...
- Exact install/restore round trip while temporarily selecting `appearanceTheme = "light"` and preserving unrelated values.
- Empty `HOME` recovery.
...
- Live verification after `Page.reload` must return version `1.2.0-beta.1` and `pass: true`.
...
- Selected image contains no fake interface controls. Branded headline text inside `panda-hero.png` is permitted because it does not impersonate a native control.
