...
- Launch the official app as a normal user process with `--remote-debugging-address=127.0.0.1` and a selected port; do not install a launchd babysitter that relaunches Codex after the user quits.
...
- Back up `appearanceTheme` and `appearanceDarkCodeThemeId`, temporarily set only `appearanceTheme = "light"`, and restore both saved values exactly. Leave `appearanceDarkChromeTheme` and unrelated TOML content untouched.
