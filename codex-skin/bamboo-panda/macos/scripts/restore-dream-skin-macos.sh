...
if [ -f "$STATE_PATH" ]; then
  stop_recorded_injector \
    || fail "The recorded skin injector identity could not be verified; restore stopped without killing it."
fi
...
if [ "$UNINSTALL" = "true" ]; then
  /bin/rm -f "$HOME/Desktop/竹影熊猫 Codex.command"
  /bin/rm -f "$HOME/Desktop/竹影熊猫 Codex - 定制.command"
  /bin/rm -f "$HOME/Desktop/竹影熊猫 Codex - 验证.command"
  /bin/rm -f "$HOME/Desktop/竹影熊猫 Codex - 恢复.command"
fi

printf '竹影熊猫已移除，Codex 原始外观配置已恢复。\n'
