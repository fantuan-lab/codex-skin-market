...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf '竹影熊猫：无法解析当前 macOS 用户目录。\n' >&2; exit 1; }
  export HOME
fi
...
process_is_live() {
  ...
  status="$(/bin/ps -p "$1" -o stat= 2>/dev/null | /usr/bin/awk '{$1=$1; print}')"
  [ -n "$status" ] || return 1
...
recorded_injector_matches() {
  ...
  local saved_start="$2"
  local saved_node="$3"
  local saved_injector="$4"
  local actual_start
  local command_line

  [ -n "$pid" ] && [ -n "$saved_start" ] && [ -n "$saved_node" ] && [ -n "$saved_injector" ] \
    && [ -n "${NODE:-}" ] && [ -n "${INJECTOR:-}" ] || return 1
  process_is_live "$pid" || return 1
  [ "$saved_node" = "$NODE" ] || return 1
  if [ "$saved_injector" != "$INJECTOR" ] \
    && [ "$saved_injector" != "$INSTALL_ROOT/scripts/injector.mjs" ]; then
    return 1
  fi
  command_line="$(/bin/ps -p "$pid" -o command= 2>/dev/null || true)"
  case "$command_line" in
    "$saved_node $saved_injector --watch"*) ;;
    *) return 1 ;;
  esac
...
stop_recorded_injector() {
  ...
  recorded_injector_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" || return 1

  /bin/launchctl remove "$INJECTOR_JOB_LABEL" >/dev/null 2>&1 || true
  process_is_live "$pid" || return 0
  actual_start="$(process_started_at "$pid")"
  [ "$actual_start" = "$saved_start" ] || return 0
  recorded_injector_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" || return 1
...
# Resolve and verify the Node runtime from the signed official Codex bundle.
ensure_node_runtime() {
  discover_codex_app
  require_macos_runtime
}
...
hot_reapply_theme() {
  ...
  # An unrecorded matching watcher cannot be authenticated safely.
  local old
  old="$(/bin/ps -axo pid=,command= | /usr/bin/awk -v inj="$INJECTOR" '
