...
if (mode === "reset-demo") {
  ...
  console.log("Restored the bundled Bamboo Panda preset.");
  process.exit(0);
}
...
const name = valueFor("name", "我的竹影熊猫主题").trim().slice(0, 80);
const tagline = valueFor("tagline", "跟着熊猫一起，把灵感、代码与效率慢慢做好。").trim().slice(0, 160);
const quote = valueFor("quote", "今天先把项目啃下来").trim().slice(0, 80);
const accent = validateHex(valueFor("accent", "#3f5b2a"), "accent");
const secondary = validateHex(valueFor("secondary", "#a8b77b"), "secondary");
const highlight = validateHex(valueFor("highlight", "#c58c45"), "highlight");

const custom = {
  schemaVersion: 1,
  id: `custom-${Date.now()}`,
  name: name || "我的竹影熊猫主题",
  brandSubtitle: "BAMBOO PANDA CODEX SKIN",
  tagline: tagline || "跟着熊猫一起，把灵感、代码与效率慢慢做好。",
  projectPrefix: "选择项目 · ",
  projectLabel: "🐼  选择项目",
  statusText: "PANDA FOCUS +1",
  quote: quote || "今天先把项目啃下来",
  image,
  colors: {
    background: "#f7f3e8",
    panel: "#fffdf6",
