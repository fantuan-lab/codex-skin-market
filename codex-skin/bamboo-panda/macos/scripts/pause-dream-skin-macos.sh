...
if [ -f "$STATE_PATH" ]; then
  stop_recorded_injector || fail "The recorded skin injector identity could not be verified; it was not stopped."
fi
...
if [ "$REMOVED" = "true" ]; then
  printf '竹影熊猫已暂停（在线皮肤已移除，Codex 保持运行）；端口 %s 可能仍处于调试模式。\n' "$PORT"
elif codex_is_running; then
  printf '竹影熊猫已暂停（注入器已停止）；端口 %s 未通过 CDP 验证，未执行在线移除。\n' "$PORT"
else
  printf '竹影熊猫已暂停（Codex 未运行）。\n'
fi
