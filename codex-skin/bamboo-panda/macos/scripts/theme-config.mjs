...
// Backup these keys so Restore can put them back. Bamboo Panda deliberately
// uses Codex's light shell while active; Restore must put the user's exact
...
if (mode === "install") {
  ...
  // Only apply non-null settings. null means "backup only / leave unchanged".
  ...
  console.log("Saved base-theme backup and temporarily selected the Codex light appearance for Bamboo Panda.");
} else {
  let backup;
  try {
