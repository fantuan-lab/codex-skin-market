...
progress() {
  printf '%s\n' "$*" >&2
  /usr/bin/osascript -e "display notification \"$*\" with title \"竹影熊猫\"" >/dev/null 2>&1 || true
}
...
/usr/bin/osascript -e 'display alert "竹影熊猫" message "主题已切换，但注入失败，请再次点击应用皮肤。"' >/dev/null 2>&1 || true
exit 1
