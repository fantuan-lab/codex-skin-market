...
if [ "$RESET_DEMO" = "true" ]; then
  ...
  if [ -z "$THEME_NAME" ]; then
    THEME_NAME="$(/usr/bin/osascript -e 'text returned of (display dialog "给这套主题起个名字" default answer "我的竹影熊猫主题" buttons {"取消", "继续"} default button "继续" with title "竹影熊猫")')" \
      || fail "Theme setup was cancelled."
  fi
  if [ -z "$TAGLINE" ]; then TAGLINE="跟着熊猫一起，把灵感、代码与效率慢慢做好。"; fi
  if [ -z "$QUOTE" ]; then QUOTE="今天先把项目啃下来"; fi
