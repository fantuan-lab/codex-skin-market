...
if (Test-Path -LiteralPath $StatePath) {
  ...
  if (-not (Stop-RecordedDreamSkinInjector $state $injector)) {
...
if ($Uninstall) {
  ...
  @(
    (Join-Path $desktop 'Codex 竹影熊猫.lnk'),
    (Join-Path $desktop 'Codex 竹影熊猫 - 恢复.lnk'),
    (Join-Path $startMenu 'Codex 竹影熊猫.lnk'),
...
if ($RestartCodex -and $RestoreBaseTheme) {
...
Write-Host '竹影熊猫的皮肤监视器已停止。Codex 的 CDP 端口会一直保留到 Codex 退出。'
