...
const EN_UI_COPY = {
  ...
  workflow: {
    ...
    steps: [
      ...
      },
      {
        ...
        copy: "Write only restricted metadata fixes, preserve the original, rerun checks, and export accessible HTML plus JSON evidence.",
      },
    ],
  },
  scope: {
  ...
  workspace: {
    ...
    revisionBoundary: "The original is never overwritten. ClearTag saves a new file, reopens it, compares protected structure signals, and reruns the analysis before download. PDF serialization can change non-content bytes, so only files that pass the safety gate are eligible.",
    ...
    busy: {
      revision: "Creating and rechecking a restricted metadata revision",
...
const ZH_UI_COPY: UiCopy = {
  ...
  workflow: {
    ...
    steps: [
      ...
      { title: "建立版本、复查并导出", copy: "只进行受限元数据修订，保留原件，重新检测并导出无障碍 HTML 与 JSON 证据。" },
    ],
  },
  scope: {
    label: "如实说明覆盖范围",
  ...
  workspace: {
    ...
    revisionBoundary: "原文件绝不会被覆盖。ClearTag 会另存新文件、重新打开、比较受保护的结构信号，并在下载前重新分析。PDF 重新序列化可能改变非内容字节，因此只有通过安全门禁的文件才允许修订。",
...
export function localizeVersionRecord(
  ...
  return {
    ...
    changes: version.changes.map((change) => changes[change] ?? change),
  };
}
