...
Analyzer eligibility is a necessary first gate, never sufficient authorization: any inconclusive safety probe or exposed out-of-scope signal disables revision creation. The real input bytes then pass an independent, fail-closed final gate using strict invalid-object handling, parser-reported invalid or unresolved references, a 50 MB / 100-page / 1,000,000-operator budget, and PDF 1.7 header preservation. Encryption, tags or MarkInfo, annotations, actions, name trees, attachments, outlines, XMP, forms/XFA, signatures/permissions, images, Form XObjects, rich media, and unsupported object types are refused. This does not claim that the parsers can recognize every possible PDF syntax defect. Tag trees, figures, lists, complex tables, formulas, OCR, and interactive forms are never auto-repaired by this MVP.
